...
class UserManager(BaseUserManager):
    def create_user(self, full_name, id_card, phone_number, password=None, **extra_fields):
        if not full_name:
    ...
    def create_superuser(self, full_name, id_card, phone_number, password, **extra_fields):
        extra_fields.setdefault("is_staff", True)
        extra_fields.setdefault("is_verified", True)
        extra_fields.setdefault("is_superuser", True)

        user = self.create_user(full_name=full_name,
                                id_card=id_card, phone_number=phone_number, password=password, **extra_fields)
        user.save(using=self.db)
        return user
